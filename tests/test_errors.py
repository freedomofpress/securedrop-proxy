...
def test_input_invalid(proxy_request):
    ...
    assert (
        ...
        == '{"error":"invalid type: string \\"foo\\", expected struct IncomingRequest at line 1 column 5"}'
    )
...
def test_input_missing_keys(proxy_request):
    ...
    assert (
        result.stderr.decode().strip()
        == '{"error":"unknown field `foo`, expected one of `method`, `path_query`, `stream`, `headers`, `body`, `timeout` at line 1 column 6"}'
    )
...
def test_cannot_connect(proxy_request):
    ...
    assert (
        result.stderr.decode().strip()
        == '{"error":"error sending request for url (http://missing.test/): error trying to connect: dns error: failed to lookup address information: Name or service not known"}'
    )
